...
from raphael.utils import strings, logger
...
class DBConfig(object):
    ...
    def from_dict(cls, _dict):
        ...
        return DBConfig(
            ...
            max_overflow=_dict.get('max_overflow', DBConfig.DEFAULT_MAX_OVERFLOW),
            pool_recycle=_dict.get('pool_recycle', DBConfig.DEFAULT_POOL_RECYCLE),
            pool_timeout=_dict.get('pool_timeout', DBConfig.DEFAULT_POOL_TIMEOUT),
        )
...
class DBContext:
    ...
    def save(self, table_name, obj):
        ...
        if ("id" not in obj) or strings.is_blank(obj["id"]):
            ...
            count = self.create_sql_query("select count(*) c from " + table_name + " where id = :id",
                                          id=obj["id"]).fetch()[0]["c"]
            insert_flag = int(count) == 0
        utcnow = datetime.utcnow()
        obj['utc_modified'] = utcnow
        if insert_flag:
            obj['utc_create'] = utcnow
